...
exports.register_a_user = async function(req, res) {
    ...
    try {
        ...
        // Validate user input
        if (fullName == "" || email == "" || password == "" || accountBalance == null) {
            console.log(fullName);
            res.status(400).send("Veuillez rentrer toutes les infos");
        }
        ...
        // // return new user
        // res.status(201).json({user: "utilisateur créer avec succes"});
...
exports.transfer_money = async function(req, res) {
    try {
        const { accountNumber, transferAmount, description, trnxType } = req.body;
        ...
        if (currentUser.accountNumber !== beneficiary) {
            ...
            let notifications = await Notifications.create({
                email: transactions[0].sender.toString(),
                trnxType: transactions[0].trnxType,
                amount: transactions[0].transactionAmount,
                sendersName: "Son nom est Kouassi",
              });
...
exports.credit_notification = async (req, res) => {
    try {
